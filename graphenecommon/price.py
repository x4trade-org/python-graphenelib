...
class Price(dict, AbstractBlockchainInstanceProvider):
    """ This class deals with all sorts of prices of any pair of assets to
        ...
        * ``Price("0.315 USD/X4T")``
        * ``Price(0.315, base="USD", quote="X4T")``
        * ``Price(0.315, base=self.asset_class("USD"), quote=self.asset_class("X4T"))``
    ...
    def invert(self):
        """ Invert the price (e.g. go from ``USD/X4T`` into ``X4T/USD``)
        """
        tmp = self["quote"]
